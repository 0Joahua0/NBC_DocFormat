...
def _configure_tk_high_dpi(root):
    ...
    try:
        ...
        root._nbc_docformat_dpi = dpi
        root._nbc_docformat_scaling = scaling
    except Exception:
        root._nbc_docformat_dpi = 96.0
        root._nbc_docformat_scaling = 96.0 / 72.0
...
class Theme:
    ...
    PRIMARY = '#25B6DB'
    PRIMARY_HOVER = '#1399BF'
    PRIMARY_LIGHT = '#DDF7FF'
    PRIMARY_SOFT = '#8EE0F2'
    ACCENT = '#0EA5B7'
    ACCENT_LIGHT = '#D9F8FB'
...
class AboutDialog(tk.Toplevel):
    """Show application and privacy information."""
...
def _get_config_dir():
    ...
    if sys.platform == 'darwin':
        ...
        config_dir = Path.home() / 'Library' / 'Application Support' / 'NBC_DocFormat'
        ...
        config_dir = Path(base) / 'NBC_DocFormat'
    else:
...
_PRIORITY_FONTS = [
    ...
    '方正小标宋简体', '方正小标宋_GBK',
    '方正黑体_GBK', '方正楷体_GBK', '方正仿宋', '方正仿宋_GBK',
    '华文仿宋', '华文中宋',
...
DEFAULT_CUSTOM_SETTINGS = {
    ...
    'title': {
        ...
        'line_spacing_type': 'exact', 'line_spacing': 28,
    ...
    'body': {
        ...
        'indent': 32, 'line_spacing_type': 'exact', 'line_spacing': 28,
        'space_before': 0, 'space_after': 0
    },
    'signature': {
        'font_cn': '仿宋_GB2312', 'font_en': 'Times New Roman',
    ...
    'table': {
        'font_cn': '仿宋_GB2312', 'font_en': 'Times New Roman',
        'size': 12, 'bold': False,
        'line_spacing_type': 'exact', 'line_spacing': 22,
...
class CustomSettingsDialog(tk.Toplevel):
    ...
    def _create_widgets(self):
        ...
        self.title_line_spacing_type_var = tk.StringVar()
        self._create_combobox(row_t, self.title_line_spacing_type_var,
                              list(LINE_SPACING_TYPE_OPTIONS.values()), width=9,
                              initial_value=self._line_spacing_type_display(self.settings.get('title', {}))).pack(side='left', padx=3)
        tk.Label(row_t, text="值:", font=get_font(11), bg=Theme.BG, fg=Theme.TEXT_SECONDARY, width=3, anchor='e').pack(side='left')
        self.title_line_spacing_var = tk.StringVar(value=self._line_spacing_value_display(self.settings.get('title', {}), 28))
        tk.Entry(row_t, textvariable=self.title_line_spacing_var, font=get_font(11), width=5, relief='solid', bd=1).pack(side='left', padx=3)
        tk.Label(row_t, text="磅/倍", font=get_font(10), bg=Theme.BG, fg=Theme.TEXT_MUTED).pack(side='left')
        ...
        tk.Label(row_h3, text="字号:", font=get_font(11), bg=Theme.BG, fg=Theme.TEXT_SECONDARY, width=5, anchor='e').pack(side='left', padx=(10, 0))
        self.h3_size_var = tk.StringVar()
        self._create_combobox(row_h3, self.h3_size_var, [f"{name}({pt}pt)" for name, pt in FONT_SIZES], width=11,
                              initial_value=self._size_display(self.settings.get('heading3', {}).get('size', 16))).pack(side='left', padx=3)

        self.h3_bold_var = tk.BooleanVar(value=self.settings.get('heading3', {}).get('bold', False))
        self._create_inline_toggle(
            row_h3, "加粗", self.h3_bold_var
        ).pack(side='left', padx=(10, 0))
        ...
        tk.Label(row_h4, text="字号:", font=get_font(11), bg=Theme.BG, fg=Theme.TEXT_SECONDARY, width=5, anchor='e').pack(side='left', padx=(10, 0))
        self.h4_size_var = tk.StringVar()
        self._create_combobox(row_h4, self.h4_size_var, [f"{name}({pt}pt)" for name, pt in FONT_SIZES], width=11,
                              initial_value=self._size_display(self.settings.get('heading4', {}).get('size', 16))).pack(side='left', padx=3)

        self.h4_bold_var = tk.BooleanVar(value=self.settings.get('heading4', {}).get('bold', False))
        self._create_inline_toggle(
            row_h4, "加粗", self.h4_bold_var
        ).pack(side='left', padx=(10, 0))
        ...
        self.line_spacing_type_var = tk.StringVar()
        self._create_combobox(row_b1, self.line_spacing_type_var,
                              list(LINE_SPACING_TYPE_OPTIONS.values()), width=9,
                              initial_value=self._line_spacing_type_display(self.settings.get('body', {}))).pack(side='left', padx=3)
        tk.Label(row_b1, text="值:", font=get_font(11), bg=Theme.BG, fg=Theme.TEXT_SECONDARY, width=3, anchor='e').pack(side='left')
        self.line_spacing_var = tk.StringVar(value=self._line_spacing_value_display(self.settings.get('body', {}), 28))
        tk.Entry(row_b1, textvariable=self.line_spacing_var, font=get_font(11), width=5, relief='solid', bd=1).pack(side='left', padx=3)
        tk.Label(row_b1, text="磅/倍", font=get_font(10), bg=Theme.BG, fg=Theme.TEXT_MUTED).pack(side='left')
        ...
        tk.Label(row_b2, text="  ⓘ 正文字体/字号同时应用于: 落款、附件、结束语",
        ...
        self.table_line_spacing_type_var = tk.StringVar()
        self._create_combobox(row_tbl1, self.table_line_spacing_type_var,
                              list(LINE_SPACING_TYPE_OPTIONS.values()), width=9,
                              initial_value=self._line_spacing_type_display(self.settings.get('table', {}))).pack(side='left', padx=3)
        tk.Label(row_tbl1, text="值:", font=get_font(11), bg=Theme.BG, fg=Theme.TEXT_SECONDARY, width=3, anchor='e').pack(side='left')
        self.table_line_spacing_var = tk.StringVar(value=self._line_spacing_value_display(self.settings.get('table', {}), 22))
        ...
        tk.Label(row_tbl1, text="磅/倍", font=get_font(10), bg=Theme.BG, fg=Theme.TEXT_MUTED).pack(side='left')
    ...
    def _on_export_preset(self):
        ...
        export_data = {
            'schema_version': CONFIG_SCHEMA_VERSION,
            'export_type': 'NBC_DocFormat-preset',
    ...
    def _create_advanced_section(self, parent, pad_x):
        ...
        tk.Label(
            self._adv_content,
            text="ⓘ 此处可逐个元素类型覆盖上方快速设置的值。行距值留空表示跟随正文行距。",
            font=get_font(9), bg=Theme.BG, fg=Theme.TEXT_MUTED
    ...
    def _create_adv_element_row(self, parent, pad_x, key, label, default_font, default_size):
        ...
        ls_type_var = tk.StringVar()
        self._create_combobox(row, ls_type_var, list(LINE_SPACING_TYPE_OPTIONS.values()), width=8,
                              initial_value=self._line_spacing_type_display(self.settings.get(key, {}))).pack(side='left', padx=3)
        ls_val = self.settings.get(key, {}).get('line_spacing', '')
        ls_var = tk.StringVar(value=str(ls_val) if ls_val else '')
        tk.Entry(row, textvariable=ls_var, font=get_font(10), width=5, relief='solid', bd=1).pack(side='left', padx=3)
        ...
        # 存储变量引用
        self._adv_vars[key] = {
            'font': font_var, 'font_en': font_en_var, 'size': size_var,
            'line_spacing_type': ls_type_var, 'line_spacing': ls_var,
            'bold': bold_var,
        }
        ...
        self._adv_initial_values[key] = {
            ...
            'size': size_var.get(),
            'line_spacing_type': ls_type_var.get(),
    ...
    def _line_spacing_type_key(self, fmt):
        """配置 dict → line_spacing_type，兼容旧版空值表示 1.5 倍行距。"""
        spacing_type = (fmt or {}).get('line_spacing_type')
        if spacing_type in LINE_SPACING_TYPE_OPTIONS:
            return spacing_type
        value = (fmt or {}).get('line_spacing')
        return 'multiple' if value in (None, '') else 'exact'

    def _line_spacing_type_display(self, fmt):
        return LINE_SPACING_TYPE_OPTIONS[self._line_spacing_type_key(fmt)]

    def _get_line_spacing_type_from_var(self, var):
        label = var.get()
        for key, value in LINE_SPACING_TYPE_OPTIONS.items():
            if value == label:
                return key
        return 'exact'

    def _line_spacing_value_display(self, fmt, fallback):
        value = (fmt or {}).get('line_spacing', fallback)
        return '' if value in (None, '') else str(value)

    def _get_line_spacing(self, var, fallback=28, spacing_type='exact'):
        """从行距输入框获取值；空值按类型回退到默认磅值或 1.5 倍。"""
        val = var.get().strip()
        if not val:
            return 1.5 if spacing_type == 'multiple' else fallback
        try:
            number = float(val)
            if number <= 0:
                return 1.5 if spacing_type == 'multiple' else fallback
    ...
    def _load_values(self):
        ...
        try:
            ...
            self.title_line_spacing_type_var.set(self._line_spacing_type_display(s.get('title', {})))
            self.title_line_spacing_var.set(self._line_spacing_value_display(s.get('title', {}), 28))
            ...
            self.h3_bold_var.set(s.get('heading3', {}).get('bold', False))
            ...
            self.h4_bold_var.set(s.get('heading4', {}).get('bold', False))
            
            # 正文
            self.body_font_var.set(s.get('body', {}).get('font_cn', '仿宋_GB2312'))
            self._set_size_var(self.body_size_var, s.get('body', {}).get('size', 16))
            self.line_spacing_type_var.set(self._line_spacing_type_display(s.get('body', {})))
            self.line_spacing_var.set(self._line_spacing_value_display(s.get('body', {}), 28))
            ...
            self.table_line_spacing_type_var.set(self._line_spacing_type_display(tbl))
            self.table_line_spacing_var.set(self._line_spacing_value_display(tbl, 22))
            ...
            for key, vars_dict in self._adv_vars.items():
                ...
                vars_dict['line_spacing_type'].set(self._line_spacing_type_display(elem))
                ...
                self._adv_initial_values[key] = {
                    ...
                    'font_en': vars_dict['font_en'].get(),
                    'size': vars_dict['size'].get(),
                    'line_spacing_type': vars_dict['line_spacing_type'].get(),
    ...
    def _save_values(self):
        ...
        title_spacing_type = self._get_line_spacing_type_from_var(self.title_line_spacing_type_var)
        body_spacing_type = self._get_line_spacing_type_from_var(self.line_spacing_type_var)
        table_spacing_type = self._get_line_spacing_type_from_var(self.table_line_spacing_type_var)
        body_ls = self._get_line_spacing(self.line_spacing_var, 28, body_spacing_type)
        title_ls = self._get_line_spacing(self.title_line_spacing_var, 28, title_spacing_type)
        table_ls = self._get_line_spacing(self.table_line_spacing_var, 22, table_spacing_type)
        try:
        ...
        self.settings = {
            ...
            'title': {
                ...
                'line_spacing_type': title_spacing_type, 'line_spacing': title_ls,
                'space_before': space_before, 'space_after': space_after
            },
            'recipient': {
                'font_cn': body_font, 'font_en': global_font_en,
                'size': body_size, 'bold': body_bold, 'align': 'left', 'indent': 0,
                'line_spacing_type': body_spacing_type, 'line_spacing': body_ls,
                'space_before': space_before, 'space_after': space_after
            },
            'heading1': {
            ...
            'heading3': {
                ...
                'size': h3_size, 'bold': self.h3_bold_var.get(), 'align': 'left', 'indent': indent_pt,
                'line_spacing_type': body_spacing_type, 'line_spacing': body_ls,
                'space_before': space_before, 'space_after': space_after
            },
            'heading4': {
                'font_cn': self.h4_font_var.get(), 'font_en': self.h4_font_en_var.get(),
                'size': h4_size, 'bold': self.h4_bold_var.get(), 'align': 'left', 'indent': indent_pt,
                'line_spacing_type': body_spacing_type, 'line_spacing': body_ls,
                'space_before': space_before, 'space_after': space_after
            },
            'body': {
            ...
            'table': {
                ...
                'line_spacing_type': table_spacing_type, 'line_spacing': table_ls,
                'first_line_indent': 0,
        ...
        for key, vars_dict in self._adv_vars.items():
            if key in self.settings and isinstance(self.settings[key], dict):
                ...
                adv_ls_type = self._get_line_spacing_type_from_var(vars_dict['line_spacing_type'])
...
class FileInputField(tk.Frame):
    ...
    def __init__(self, parent, label_text, placeholder, variable, command, drop_command=None, **kwargs):
        ...
        # 悬停效果
        self.container.bind('<Enter>', lambda e: self.container.configure(highlightbackground=Theme.BORDER_HOVER))
...
class SelectableCard(tk.Frame):
    ...
    def __init__(self, parent, title, description, value, variable,
                 icon_draw_func=None, icon_path=None, is_featured=False, command=None, **kwargs):
        ...
        # 左侧：图标
        if icon_path:
            target_size = ui_px(76 if is_featured else 64)
            self._icon_image = self._load_icon_image(icon_path, target_size)
        ...
        if self._icon_image is not None:
            self.icon_label = tk.Label(
                ...
                bg=bg_color,
                width=self._icon_image.width(),
                height=self._icon_image.height(),
            )
            self.icon_label.pack(side='left', padx=(0, Theme.SPACE_MD))
    ...
    def _load_icon_image(self, icon_path, target_size):
        try:
            image = tk.PhotoImage(file=str(icon_path))
        except tk.TclError:
            return None

        self._icon_source_image = image
        base = max(1, min(image.width(), image.height()))
        factor = max(1, base // max(1, target_size))
        if factor > 1:
    ...
    def _on_enter(self, event=None):
        if not self.selected:
            self.configure(highlightbackground=Theme.BORDER_HOVER)
...
class CollapsibleLog(tk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        # 折叠条
        self.toggle_bar = tk.Frame(self, bg=Theme.BORDER_LIGHT, height=36)
...
class DocFormatApp:
    ...
    def create_widgets(self):
        ...
        smart_card = SelectableCard(
            ...
            icon_path=resource_path('assets', 'feature_smart.png'),
            is_featured=True,
        ...
        diag_card = SelectableCard(
            ...
            icon_path=resource_path('assets', 'feature_analyze.png'),
            command=self._on_mode_change
        )
        ...
        punct_card = SelectableCard(
            ...
            icon_path=resource_path('assets', 'feature_punctuation.png'),
            command=self._on_mode_change
        )
    ...
    def _draw_scrollbar(self):
        ...
        self.scrollbar_canvas.create_rectangle(
            ...
            fill=Theme.BORDER_LIGHT, outline='', tags='track'
        )
        
        # 滑块（更深的颜色）
        self.scrollbar_canvas.create_rectangle(
            4, bar_y, w - 4, bar_y + bar_h,
            fill=Theme.TEXT_MUTED, outline='', tags='thumb'
        )
    ...
    def _run_format(self, input_path, output_path, progress_callback=None, revision_mode=False):
        ...
        formatter_logger = logging.getLogger('NBC_DocFormat.formatter')
