...
def test_custom_settings_expose_each_heading_font_choice_in_the_ui():
    source = (PROJECT_ROOT / "NBC_DocFormat.py").read_text(encoding="utf-8")
    for level in range(1, 5):
        assert f"self.h{level}_font_var = tk.StringVar()" in source
        assert f"self.h{level}_font_en_var = tk.StringVar()" in source
        assert f"self.h{level}_size_var = tk.StringVar()" in source
        assert f"self.h{level}_bold_var = tk.BooleanVar" in source
    assert "if vars_dict['bold'].get() != key_initial.get('bold', False):" in source
...
def test_each_heading_writes_chinese_and_english_fonts_to_ooxml(tmp_path):
    ...
    expected_sizes = (14, 15, 16, 18)
    expected_bold = (True, False, True, False)
    for level in range(1, 5):
        custom[f"heading{level}"] = {
            **custom[f"heading{level}"],
            "font_cn": f"Chinese{level}",
            "font_en": f"Latin{level}",
            "size": expected_sizes[level - 1],
            "bold": expected_bold[level - 1],
        }
    ...
    for level, heading in enumerate(headings, start=1):
        ...
        assert paragraph.runs[0].font.size.pt == expected_sizes[level - 1]
        assert paragraph.runs[0].font.bold is expected_bold[level - 1]
