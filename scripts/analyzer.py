...
try:
    from scripts.punctuation import MIDDLE_DOT, middle_dot_run_has_required_font
except ModuleNotFoundError:  # Support direct execution from scripts/.
    from punctuation import MIDDLE_DOT, middle_dot_run_has_required_font
...
def analyze_punctuation(doc):
    ...
    for i, para in enumerate(doc.paragraphs):
        ...
        for run in para.runs:
            if MIDDLE_DOT in run.text and not middle_dot_run_has_required_font(run):
                issues.append({'para': i + 1, 'type': '间隔号字体不规范', 'char': MIDDLE_DOT})
                break
